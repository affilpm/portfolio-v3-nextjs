...
export function OpenToWorkBanner() {
  ...
  return (
    <div className="w-full bg-[var(--surface-2)] border-y border-[var(--border)] backdrop-blur-md relative z-20">
      <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm font-medium">
        <p className="text-[var(--text-primary)] max-w-lg text-center sm:text-left">
          {textMessage}
        </p>
        <div className="flex items-center gap-3 shrink-0">
          <Link
            href="/resume"
            className="px-4 py-2 border border-[var(--border)] hover:border-[var(--accent)] bg-[var(--surface)] rounded-full text-xs font-medium text-[var(--text-primary)] transition-colors"
          >
            View Resume
          </Link>
          <Link
            href="/contact"
            className="px-4 py-2 bg-[var(--accent)] text-white rounded-full text-xs font-medium hover:bg-[var(--accent)]/90 transition-colors shadow-[0_0_15px_rgba(124,58,237,0.3)]"
          >
            Contact Me
          </Link>
        </div>
